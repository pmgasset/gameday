...
Pool business time is `America/New_York`; stored timestamps are `timestamptz` UTC. A pool week is the official NFL week the provider assigned to each game (`games.nfl_season`, `games.nfl_week`), never a calendar week: `nfl_week_sunday` anchors the week on the Eastern Sunday its slate is played, so a Wednesday or Thursday opener and the Monday nighter that closes the same NFL week open and lock together. The whole week publishes at Tuesday 9 AM ET before that Sunday, and each game's effective deadline is `min(kickoff, Sunday 1 PM ET)`. The RLS reveal helper and submission RPC independently enforce reveal and deadline rules from the same week helpers, and `lib/domain/deadlines.ts` derives the identical window from the week's slate for rendering. Browser countdowns are informational only.
