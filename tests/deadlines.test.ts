...
import { effectivePickDeadline, formatEastern, isPickAvailable, isPickLocked, sundayGlobalDeadline, weeklyPicksOpenAt } from "@/lib/domain/deadlines";
...
describe("authoritative pool deadlines", () => {
  ...
  it("publishes a week's picks Tuesday at 9 AM Eastern", () => { const mondayGame=new Date("2026-10-20T00:15:00Z"); const sundayGame=new Date("2026-10-18T20:25:00Z"); expect(weeklyPicksOpenAt(mondayGame).toISOString()).toBe("2026-10-13T13:00:00.000Z"); expect(weeklyPicksOpenAt(sundayGame).toISOString()).toBe("2026-10-13T13:00:00.000Z"); expect(isPickAvailable(sundayGame,new Date("2026-10-13T12:59:59Z"))).toBe(false); expect(isPickAvailable(sundayGame,new Date("2026-10-13T13:00:00Z"))).toBe(true); });
