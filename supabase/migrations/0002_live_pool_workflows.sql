...
declare v_token text := encode(gen_random_bytes(32),'hex'); v_id uuid; v_actor uuid := auth.uid();
begin
  if not is_pool_commissioner(p_pool_id) then raise exception 'Only commissioners can create invitations'; end if;
  insert into invitations(pool_id,token_hash,created_by,expires_at) values(p_pool_id,encode(digest(v_token,'sha256'),'hex'),v_actor,p_expires_at) returning invitations.id into v_id;
...
begin
  if v_actor is null then raise exception 'Authentication required'; end if;
  select * into v_invitation from invitations where token_hash=encode(digest(p_token,'sha256'),'hex') and revoked_at is null and (expires_at is null or expires_at > now());
